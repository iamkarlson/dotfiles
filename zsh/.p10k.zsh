# Generated by Powerlevel10k configuration wizard on 2023-02-27 at 13:49 CET.
...
# 24h time, angled separators, round heads, sharp tails, 2 lines, solid, right frame,
...
() {
  ...
  # Green prompt symbol if the last command succeeded (purple if SSH).
  ...
  #####################################[ vcs: git status ]######################################
  # Version control background colors (purple if SSH, green if local).
  if [[ -n "$SSH_CONNECTION" ]] || [[ -n "$SSH_CLIENT" ]]; then
  ...
  # Hide context when inside tmux (even in SSH sessions)
  typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_CONTENT_EXPANSION='${${TMUX:+}:-"%n@%m"}'
